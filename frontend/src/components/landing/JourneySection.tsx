...
import { Scan, Code2, Send, Brain, BarChart3 } from "lucide-react";
...
export function JourneySection() {
  ...
  const steps = [
    { num: 1, title: "Capture Problem", desc: "One click from Codeforces, CodeChef, or CSES.", icon: <Scan className="w-5 h-5 text-zinc-900" />, color: "bg-emerald-400" },
    { num: 2, title: "Solve Locally", desc: "Code in a clean, distraction-free environment.", icon: <Code2 className="w-5 h-5 text-zinc-900" />, color: "bg-blue-400" },
    { num: 3, title: "Submit", desc: "Submit directly. No context switching.", icon: <Send className="w-5 h-5 text-zinc-900" />, color: "bg-indigo-400" },
    { num: 4, title: "Learn with AI", desc: "Get hints and explanations from our LangGraph agents.", icon: <Brain className="w-5 h-5 text-zinc-900" />, color: "bg-purple-400" },
    { num: 5, title: "Track Progress", desc: "Watch your ratings climb across platforms.", icon: <BarChart3 className="w-5 h-5 text-zinc-900" />, color: "bg-teal-400" }
  ];

  return (
    <section ref={containerRef} id="how-it-works" className="relative w-full py-24">
      <div className="px-6 md:px-12 max-w-4xl mx-auto">
        <div className="text-center mb-16">
        <h2 className="text-3xl md:text-4xl font-outfit font-bold tracking-tight text-white mb-4">
          How it works
        </h2>
        <p className="text-zinc-400">The seamless workflow of competitive programming.</p>
      </div>
