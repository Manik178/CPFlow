...
# We will use gpt-oss-120b which has a massive context limit
# and smaller models for simpler tasks to avoid Groq's 12,000 TPM limit on the free tier.
def get_large_llm():
    return ChatGroq(model_name="gpt-oss-120b", temperature=0.2)

def get_fast_llm():
    return ChatGroq(model_name="gpt-oss-20b", temperature=0.2)
